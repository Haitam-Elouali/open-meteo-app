...
test('changing one setting does not reset the others on reload (language)', async () => {
  ...
  assert.match(window.document.querySelector('.wind').textContent, /kt/, 'knots applied');
});
...
test('wind unit conversion is applied on the card', async () => {
  ...
  // 10 km/h -> 5.4 kt -> round 5
  assert.match(kn.document.querySelector('.wind').textContent, /^5 kt$/);
