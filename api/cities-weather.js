      ...
      try {
          ...
          if (latestHour >= 18) {
            ...
            for (let i = 0; i < n; i++) {
              ...
              if (Number.isFinite(v)) {
                ...
                if (!isNaN(t) && t <= now) {
                  if (bestMax === null || v > bestMax) bestMax = v;
                  if (bestMin === null || v < bestMin) bestMin = v;
                }
              }
            }
            maxTemp = bestMax !== null ? Math.round(bestMax) : null;
            minTemp = bestMin !== null ? Math.round(bestMin) : null;
          } else if (dailyMin.length > 0 || dailyMax.length > 0) {
            if (dailyMin.length > 0) {
              const observedMin = dailyMin[0];
              minTemp = Number.isFinite(observedMin) ? Math.round(observedMin) : null;
            }
            if (dailyMax.length > 0) {
              const observedMax = dailyMax[0];
        ...
        if (maxTemp === null || minTemp === null) {
          ...
          try {
            ...
            if (minTemp === null && Number.isFinite(observedMin)) {
              minTemp = Math.round(observedMin);
            }
            if (maxTemp === null && Number.isFinite(observedMax)) {
              maxTemp = Math.round(observedMax);
            }
          } catch (e) {
        ...
        return {
          ...
          lat: loc.latitude,
          lon: loc.longitude,
          maxTemp: maxTemp !== null && Number.isFinite(maxTemp) ? Math.round(maxTemp) : null,
          minTemp: minTemp !== null && Number.isFinite(minTemp) ? Math.round(minTemp) : null,
        };
      } catch (e) {
