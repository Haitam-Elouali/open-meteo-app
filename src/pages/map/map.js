(function () {
    ...
    async function fetchGrid() {
        ...
        // Fetch all variables for all layers
        var allVars = [
            'temperature_2m', 'relative_humidity_2m', 'precipitation',
            'pressure_msl', 'cloud_cover', 'wind_speed_10m', 'wind_direction_10m',
            'uv_index', 'dew_point_2m', 'apparent_temperature', 'visibility', 'weather_code'
        ];
        allVars.forEach(function (v) { params.append('hourly', v); });
        ...
        // Update EnhancedTileManager with new grid data
        if (window.EnhancedTileManager) {
            EnhancedTileManager.setGridData(state.grid, state.times);
            EnhancedTileManager.setFrameIndex(state.frameIndex);
            var activeKeys = Object.keys(state.activeLayers).filter(function (k) { return state.activeLayers[k]; });
            EnhancedTileManager.setActiveLayers(activeKeys);
            EnhancedTileManager.renderFromGridData();
        }
    }
    ...
    function showFrame(hidx) {
        ...
        // Update EnhancedTileManager frame
        if (window.EnhancedTileManager) {
            EnhancedTileManager.setFrameIndex(hidx);
            EnhancedTileManager.renderFromGridData();
        }

        // Update animation slider
        var animSlider = document.getElementById('map-anim-slider');
        var animTime = document.getElementById('map-anim-time');
        if (animSlider && state.times.length) {
            animSlider.max = state.times.length - 1;
            animSlider.value = hidx;
        }
        if (animTime && state.times[hidx]) {
            animTime.textContent = new Date(state.times[hidx]).toLocaleString(undefined, { weekday: 'short', hour: '2-digit', minute: '2-digit' });
        }
    }
    ...
    function initControls() {
        ...
        // Multi-layer checkboxes
        var layerChecks = document.querySelectorAll('.map-layer-item input[type="checkbox"]');
        layerChecks.forEach(function (cb) {
            cb.addEventListener('change', function () {
                var layer = cb.id.replace('layer-', '');
                if (cb.checked) {
                    state.activeLayers[layer] = true;
                    if (window.EnhancedTileManager) {
                        EnhancedTileManager.setLayerVisible(layer, true);
                    }
                } else {
                    delete state.activeLayers[layer];
                    if (window.EnhancedTileManager) {
                        EnhancedTileManager.setLayerVisible(layer, false);
                    }
                }
                updateLegend();
                fetchGrid();
        ...
        var opacitySliders = document.querySelectorAll('.layer-opacity');
        opacitySliders.forEach(function (slider) {
            slider.addEventListener('input', function () {
                var layer = slider.dataset.layer;
                var val = parseInt(slider.value, 10) / 100;
                state.opacities[layer] = val;
                if (window.EnhancedTileManager) {
                    EnhancedTileManager.setLayerOpacity(layer, val);
                }
            });
        });

        // Animation controls
        var animPlay = document.getElementById('map-anim-play');
        var animSlider = document.getElementById('map-anim-slider');
        var animSpeed = document.getElementById('map-anim-speed');
        var animTime = document.getElementById('map-anim-time');

        if (animPlay && window.EnhancedTileManager) {
            animPlay.addEventListener('click', function () {
                var isPlaying = EnhancedTileManager.toggle();
                animPlay.textContent = isPlaying ? '⏸' : '▶';
            });
        }

        if (animSlider && window.EnhancedTileManager) {
            animSlider.addEventListener('input', function () {
                var idx = parseInt(animSlider.value, 10);
                EnhancedTileManager.setFrameIndex(idx);
                if (animTime && state.times[idx]) {
                    animTime.textContent = new Date(state.times[idx]).toLocaleString(undefined, { weekday: 'short', hour: '2-digit', minute: '2-digit' });
                }
            });
        }

        if (animSpeed && window.EnhancedTileManager) {
            animSpeed.addEventListener('change', function () {
                var speed = parseInt(animSpeed.value, 10);
                EnhancedTileManager.setPlaySpeed(speed);
            });
        }

        // Base map switcher
        var baseBtns = document.querySelectorAll('.map-base-btn');
    ...
    function init() {
        ...
        if (window.EnhancedTileManager) {
            EnhancedTileManager.init(map);
            EnhancedTileManager.setActiveLayers(['temperature']);
        }
        ...
        var tempCheck = document.getElementById('layer-temperature');
        if (tempCheck) tempCheck.checked = true;
