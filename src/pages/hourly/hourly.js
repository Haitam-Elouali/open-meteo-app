  ...
  function drawChart() {
    ...
    fetch(`/api/hourly?lat=${lat}&lon=${lon}`)
      ...
      .then((res) => {
        ...
        // Find the FIRST occurrence of the current hour; that is now's slot.
        let currentIdx = -1;
        for (let i = 0; i < times.length; i++) {
          const d = new Date(times[i]);
          if (d.getHours() === currentHour) { currentIdx = i; break; }
        }
        if (currentIdx < 0) currentIdx = 0;

        const maxPoints = 25;
        const entries = [];
        for (let i = currentIdx; i < times.length && entries.length < maxPoints; i++) {
          entries.push({
            time: times[i],
            temp: temps[i],
            precip: precips[i],
        ...
        if (rangeEl && entries.length) {
          ...
          const s = fmt(first);
          const e = fmt(last);
          rangeEl.textContent = s && e ? (s === e ? s : `${s} → ${e}`) : '';
        }
        ...
        function render() {
          ...
          entries.forEach((entry, i) => {
            ...
            // x labels. Show the actual hour for each entry in the past-24h window.
          ...
          // Explicit 24h window caption, e.g. "2026-07-29 → 2026-07-30 (24h)".
          const firstEntry = entries.length ? new Date(entries[0].time) : null;
          const lastEntry = entries.length ? new Date(entries[entries.length - 1].time) : null;
          const fmtDate = (d) => {
            if (!d || isNaN(d)) return '';
            return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
          };
          const firstEntryHour = firstEntry ? pad2(firstEntry.getHours()) + ':00' : '';
          const lastEntryHour = lastEntry ? pad2(lastEntry.getHours()) + ':00' : '';
          const datePart = (() => {
            const s = fmtDate(firstEntry);
            const e = fmtDate(lastEntry);
            return s && e ? (s === e ? s : `${s} → ${e}`) : '';
          })();
          const windowText = datePart ? `${datePart}  ${firstEntryHour} → ${lastEntryHour} (24h)` : `${firstEntryHour} → ${lastEntryHour} (24h)`;
